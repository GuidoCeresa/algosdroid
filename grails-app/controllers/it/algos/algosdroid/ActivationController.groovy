...
class ActivationController {
     ...
     */
    def regolaFiltro() {
        def risultato = Pref.getBool("usaFilterActivation")
     ...
     */
    def regolaExport() {
        def risultato = Pref.getBool("usaExportActivation")
    ...
    def filter ={
        ...
        recordsParziali = filterPaneService.count(params, Activation)
        recordsTotali = Activation.count()

        //--calcola il numero di record
        titoloLista = "Elenco di " + params.max + "/" + recordsParziali + " records di activation"
        titoloListaFiltrata = "Elenco di " + params.max + "/" + recordsParziali + " records filtrati di activation"

        //--presentazione della view (index), secondo il modello
        //--menuExtra e campiLista possono essere nulli o vuoti
        //--se campiLista è vuoto, mostra tutti i campi (primi 12)
        render(view: 'index',
                model: [activationInstanceList  : filterPaneService.filter(params, Activation),
                        activationInstanceCount : recordsParziali,
                        activationInstanceTotal : recordsTotali,
    ...
    def export = {
        ...
        if (exportService && USA_EXPORT) {
            if (params?.format && params.format != 'html') {
                if (!records) {
                    records = Activation.list(params)
                }// fine del blocco if
                if (!fields) {
                    fields = new DefaultGrailsDomainClass(Activation.class).persistentProperties*.name
                }// fine del blocco if
                parameters = [title: titleReport]
                response.contentType = grailsApplication.config.grails.mime.types[params.format]
                response.setHeader("Content-disposition", "attachment; filename=Activation.${params.extension}")
    ...
    def show(Long id) {
        def activationInstance = Activation.get(id)
        ...
        if (!activationInstance) {
            flash.message = message(code: 'default.not.found.message', args: [message(code: 'activation.label', default: 'Activation'), id])
            redirect(action: 'list')
            return
        }// fine del blocco if e fine anticipata del metodo

        render(view: 'show',
                model: [activationInstance : activationInstance,
    ...
    def delete(Long id) {
        ...
        try {
            ...
            flash.message = message(code: 'default.deleted.message', args: [message(code: 'activation.label', default: 'Activation'), id])
            redirect(action: 'list')
        }// fine del blocco try
        catch (DataIntegrityViolationException e) {
            flash.message = message(code: 'default.not.deleted.message', args: [message(code: 'activation.label', default: 'Activation'), id])
    ...
    private Class getClasse() {
        return LibGrails.getDomainClazz(grailsApplication, 'Activation')
    } // fine del metodo
